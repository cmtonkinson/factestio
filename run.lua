...
if FACTESTIO_ROOT:sub(-1) ~= '/' then FACTESTIO_ROOT = FACTESTIO_ROOT .. '/' end
...
parser:flag('--on')
  :description('Enable factestio for this mod project (symlink, mod-list, scaffold)')
parser:flag('--off')
  :description('Disable factestio for this mod project')
parser:flag('-q --quiet')
  :description('Suppress informational output (use with --on/--off)')
parser:flag('-d --debug')
...
parser:option('-t --timeout')
  ...
  :default('8')
  :convert(tonumber)
parser:argument('mod_dir')
  :description('Mod project directory (default: current directory)')
  :default('./')
  :args('?')
...
if mod_dir:sub(-1) ~= '/' then mod_dir = mod_dir .. '/' end
...
if configuration and configuration.os_paths and configuration.os_paths.data then
  ...
  if data_path:sub(-1) ~= '/' then data_path = data_path .. '/' end
end
...
if args.on then
  ...
  if target then
    if abs_target ~= abs_expected then
      ...
      if not quiet then print('factestio symlink already correct') end
    end
  else
    os.execute('ln -sf "' .. abs_expected .. '" "' .. link_path .. '"')
    if not quiet then print('Created symlink: ' .. link_path .. ' -> ' .. abs_expected) end
  end
  ...
  if data_path then
    ...
    if mods_target then
      if not quiet then print('Mod symlink already exists: ' .. mods_link) end
    else
      os.execute('ln -sf "' .. FACTESTIO_ROOT:gsub('/$', '') .. '" "' .. mods_link .. '"')
      if not quiet then print('Created mod symlink: ' .. mods_link) end
    end

    -- 3. Enable in mod-list.json
    set_mod_enabled(data_path, true, quiet)
  else
    if not quiet then print('Note: skipping mod-list.json (no config found yet)') end
  end
  ...
  if not realpath(factestio_dir) then
    ...
    if not quiet then print('Created directory: ' .. factestio_dir) end
  end
  ...
  if not exists(config_dst) then
    ...
    if not quiet then print('Created: ' .. config_dst) end
  end
  ...
  if not exists(example_dst) then
    ...
    if not quiet then print('Created: ' .. example_dst) end
  end
...
if args.off then
  ...
  if target then
    ...
    if not quiet then print('Removed symlink: ' .. link_path) end
  end
  ...
  if data_path then
    ...
    if mods_target then
      ...
      if not quiet then print('Removed mod symlink: ' .. mods_link) end
    end
