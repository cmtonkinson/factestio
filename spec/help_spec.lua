...
describe("factestio --help", function()
  it("prints a real help screen", function()
    ...
    -- The aliases are only discoverable if the help screen names them.
    assert.is_truthy(output:find("activate, on", 1, true))
    assert.is_truthy(output:find("deactivate, off", 1, true))
    assert.matches("list", output)
    assert.matches("%-%-leaf", output)
    assert.matches("%-%-branch", output)
    assert.matches("%-%-roots", output)
    assert.matches("%-%-children", output)
    assert.matches("%-%-json", output)
