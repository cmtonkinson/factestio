...
function F.start_factorio(node, depth)
  ...
  -- Write the test name into settings.lua. We don't need/use settings.lua
  -- for anything else, so it's not checked into git and only eixsts for this
  -- purpose.
  local content = [[-- This file is generated by factestio. Do not edit.
    data:extend({
      {
        type = "string-setting",
        name = "factestio-test-name",
        setting_type = "startup",
        default_value = "]] .. node.name .. [[",
      }
    })]]
  local file = assert(io.open(settings_file, 'w'))
  file:write(content)
  file:close()

  -- Start the headless scenario in the background.
  F.cmd('%s --start-server-load-scenario factestio/factestio --server-settings "%s" --disable-audio --nogamepad 2>&1 &'
    , F.FACTORIO_BINARY
    , F.SETTINGS
  )
